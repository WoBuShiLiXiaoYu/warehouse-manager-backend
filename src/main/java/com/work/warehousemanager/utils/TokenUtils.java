...
import com.work.warehousemanager.exception.BusinessException;
...
public class TokenUtils {
    ...
    public String loginSign(CurrentUser currentUser, String password){
        ...
        //将 token 保存到 redis 中,并设置 token 在 redis 中的过期时间
        stringRedisTemplate.opsForValue().set(RedisConstants.LOGIN_TOKEN_KEY + token, token, expireTime * 2, TimeUnit.SECONDS);
        return token;
    }
    ...
    public CurrentUser getCurrentUser(String token) {
        ...
        //从解码后的 token 中获取用户信息并封装到 CurrentUser 对象中返回
